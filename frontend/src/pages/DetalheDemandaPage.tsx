      ...
      <div className="sticky top-0 z-20 bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative">
            <button
              onClick={() => setShowModulosMenu(v => !v)}
              className="flex items-center gap-1 px-3 py-1.5 border border-blue-200 text-blue-700 rounded-lg text-xs font-medium hover:bg-blue-50 transition-colors"
              aria-label="Ver módulos da demanda"
            >
            ...
            {showModulosMenu && (
              <>
                <div className="fixed inset-0 z-10" onClick={() => setShowModulosMenu(false)} />
                <div className="absolute left-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-20 w-72 overflow-hidden">
                  <div className="px-3 py-2 border-b border-slate-100 bg-slate-50">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400">Módulos da Demanda</p>
                  </div>
                  <div className="grid grid-cols-2 gap-px bg-slate-100">
                    {[
                      { label: 'Canvas',        path: 'canvas',        mod: 'M3'  },
                      { label: 'Requisitos',    path: 'requisitos',    mod: 'M4'  },
                      { label: 'Backlog',       path: 'backlog',       mod: 'M5'  },
                      { label: 'Casos de Uso',  path: 'casos-de-uso',  mod: 'M6'  },
                      { label: 'Modelagem',     path: 'modelagem',     mod: 'M7'  },
                      { label: 'Prototipação',  path: 'prototipo',     mod: 'M8'  },
                      { label: 'Arquitetura',   path: 'arquitetura',   mod: 'M9'  },
                      { label: 'Estimativas',   path: 'estimativas',   mod: 'M10' },
                      { label: 'Governança',    path: 'governanca',    mod: 'M11' },
                      { label: 'Conformidade',  path: 'conformidade',  mod: 'M12' },
                      { label: 'Agentes IA',    path: 'agentes',       mod: 'M14' },
                      { label: 'Artefatos',     path: 'artefatos',     mod: ''    },
                    ].map(({ label, path, mod }) => (
                      <button key={path}
                        onClick={() => { navigate(`/demandas/${id}/${path}`); setShowModulosMenu(false) }}
                        className="flex items-center gap-2 px-3 py-2.5 bg-white hover:bg-blue-50 text-left transition-colors">
                        {mod && <span className="text-[9px] font-bold text-blue-500 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded shrink-0">{mod}</span>}
